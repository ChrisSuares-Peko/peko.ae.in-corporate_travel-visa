...
function HotelCard({ isMobile }) {
  ...
  return (
    <div style={{ padding: isMobile ? '20px 16px 24px' : '24px 32px 32px' }}>
      ...
      {/* Row 2: Nationality / Residence / Search CTA */}
      <div style={{
        display: 'grid',
...
function VisaCard({ isMobile }) {
  ...
  return (
    <div style={{ padding: isMobile ? '20px 16px 24px' : '24px 32px 32px' }}>
      ...
      {/* Row 1: Nationality / Residence / Destination / Travel Date */}
      <div style={{
        display: 'grid',
        ...
        <div>
          ...
          <select
            ...
            style={{ ...inputBase, cursor: 'pointer', color: form.destination ? '#1A1A1A' : '#8A8A8A' }}
            onFocus={focusRed} onBlur={blurGrey}
          >
            ...
            {['Tourist', 'Business'].map(type => (
              <label
                ...
                style={{
                  ...
                  fontSize: 15,
                  color: '#1A1A1A',
                  fontWeight: form.visaType === type ? 600 : 400,
                }}
              >
                <input
                  type="radio"
...
function ComingSoonCard({ icon, label, description }) {
  return (
    <div style={{ padding: '72px 32px', textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 14, color: '#8A8A8A', maxWidth: 400, margin: '0 auto 28px', lineHeight: 1.65 }}>
        {description}
      </div>
      <div style={{
        ...
        <div style={{
          ...
          {activeTab === 'air'   && (
            <ComingSoonCard
              icon="✈️"
              label="Air Tickets"
              description="Book flights for your team's corporate travel needs. Fast, easy, and fully managed from one platform."
            />
          )}
          {activeTab === 'esim'  && (
            <ComingSoonCard
              icon="📱"
              label="Travel eSIM"
              description="Stay connected worldwide with instant eSIM activation — no physical SIM card needed."
            />
          )}
        </div>
